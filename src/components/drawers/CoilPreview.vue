    ...
    <v-container>
        ...
        <v-col
          ...
          v-if="
            (checkRole('member') &&
              ...
              $route.path === '/slits-preview') ||
            (checkRole('admin') && item.status === 'require approval') ||
            item.status == 'slitted'
          "
        >
          <v-text-field
      ...
      <v-row v-if="rows.length > 0" class="px-4">
        <v-col
          ...
          v-if="
            (checkRole('member') &&
              rows[0].status === 'in-queue' &&
              $route.path === '/slits-preview') ||
            (checkRole('admin') && rows[0].status === 'require approval')
          "
        >
          <v-textarea
            v-model="notes"
...
export default {
  ...
  methods: {
    ...
    async saveCoil() {
      ...
      if (this.checkRole("member") && status == "in-queue")
