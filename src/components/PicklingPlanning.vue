      ...
      >
        <v-card>
          ...
          <v-card-text>
            <v-container>
              <v-row
                v-for="(item, index) in $store.state.selSlits"
                :key="index"
              >
...
export default {
  ...
  computed: {
    formTitle() {
      if (this.$route.path === "/pickling-stock")
        return "Create Pickling Planning";
  ...
  methods: {
    ...
    async savePlanning() {
      ...
      this.$store.state.selSlits.map((item) => {
        data.push({
          pickling_shift: item.pickling_shift,
          pickling_date: item.pickling_date,
          status: "in-process for pickling",
          id: item.ID,
      ...
      try {
        ...
        let payload = {
          status: "in-process for pickling,approve for pickling",
        };
        if (this.$route.path == "/pickling-stock")
          payload = { status: "slitted" };
