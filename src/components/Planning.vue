          ...
          <template>
            <v-row class="pb-3">
              ...
              <v-col cols="6" class="my-3"
                ><h3>Coil {{ coilData.brand_no }}</h3></v-col
              >
              <v-col
                class="text-right"
                v-if="savedData.status.toLowerCase() === 'available'"
              >
                <v-btn
                  class="mr-4"
        ...
        <v-expansion-panel-content>
          <v-row>
            <v-col cols="4">
              ...
              <p class="mb-2">
                Billed Weight: {{ coilData.formulated_weight }} kg
              </p>
                    ...
                    <v-col>
                      ...
                      <div class="red--text caption">
                        {{
                          validateForm
                            ? ""
                            : "Total Slitted width cannot exceed the total Coil width"
                        }}
                      </div>
                    </v-col>
                  </v-row>
...
export default {
  ...
  methods: {
    savePlanning() {
      if (this.$route.params.id) this.updateSlits();
      else this.saveSlit();
    },
    formulatedWt(width) {
      return parseFloat(
        ((this.coilData.weight / this.coilData.width) * width).toFixed(3)
      );
    },
    async updateSlits() {
      let data = {};
      ...
      if (this.avilableWidth > 0) {
        data.slittedItems.push({
          id: this.slittedItems.length + 1,
          status: "in-queue",
          is_avilable: true,
          created_at: calendarDate,
          updated_at: calendarDate,
          slitted_width: this.avilableWidth,
          slitted_weight: this.formulatedWt(this.avilableWidth),
          parent_id: this.coilData.id,
        });
      }
    ...
    copySlit(item) {
      ...
      this.slittedItems.push({
        id: this.slittedItems.length + 1,
        slitted_width: obj.slitted_width,
        slitted_weight: obj.slitted_weight,
      });
    },
    addRule() {
      this.slittedItems.push({
