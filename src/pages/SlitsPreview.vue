  ...
  <v-container fluid>
    ...
    >
      <template v-slot:[`item.slit_date`]="{ item }">
        <div class="caption">
          <span>{{
            item.slit_date ? $options.filters.formatDate(item.slit_date) : "---"
          }}</span>
        </div>
      </template>
          ...
          <v-col>
            <v-row>
              ...
              <v-col cols="6" class="py-0">
                <!-- <span>Width: {{slit.slitted_width}} mm</span> -->
                <span v-if="slit.actual_width"
                  >Actual Width: {{ slit.actual_width }} mm</span
                >
              </v-col>
              <v-col cols="6" class="py-0">
                <!-- <span>Weight: {{slit.slitted_weight}} kg</span> -->
                <span v-if="slit.actual_weight"
                  >Actual Weight: {{ slit.actual_weight }} kg</span
                >
              </v-col>
            </v-row>
          </v-col>
      ...
      <template v-slot:[`item.actions`]="{ item }">
        <v-row align="center">
          <v-col
            v-if="checkRole('admin') && item.status === 'in-queue'"
            @click="openDrawer(item)"
            cols="12"
            class="pb-0"
            ><v-btn small outlined class="caption">Edit Planning</v-btn></v-col
          >
          <v-col
            v-if="checkRole('admin') && item.status === 'in-queue'"
            @click="resetCoil(item)"
            cols="12"
            ><v-btn small outlined class="caption"
              >Reset to Available</v-btn
            ></v-col
          >
          <v-col
            v-if="
              ...
              new Date(item.slit_date).getTime() <= new Date().getTime()
            "
            @click="openDrawer(item)"
            cols="12"
            ><v-btn small outlined class="caption">View & Process</v-btn></v-col
          >
          <v-col
          ...
          <tr v-for="item in selMultiRows" :key="item.name">
            ...
            <td>
              <div v-for="slit in item.slits" :key="slit.id">
                <div class="row">
                  <div class="col">Coil No: {{ slit.slit_no }}</div>
                  <div class="col">Width: {{ slit.slitted_width }} mm</div>
                  <div class="col">Weight: {{ slit.slitted_weight }} kg</div>
...
export default {
          ...
          if (!existItem) {
            ...
            let parentCoil = {
              ...
              slits: [
                {
                  id: coil.ID,
                  slitted_weight: coil.slitted_weight,
                  slitted_width: coil.slitted_width,
                  actual_weight: coil.actual_weight,
                  actual_width: coil.actual_width,
                  slit_no: coil.slit_no,
                },
              ],
            };
            slits.push(parentCoil);
          } else {
            existItem.slits.push({
              id: coil.ID,
              slitted_weight: coil.slitted_weight,
              slitted_width: coil.slitted_width,
              actual_weight: coil.actual_weight,
  ...
  methods: {
    ...
    async resetCoil(item) {
      ...
      // let ids = null
      item.slits.map((val) => {
        if (data.ids) data.ids = `${data.ids},${val.id}`;
